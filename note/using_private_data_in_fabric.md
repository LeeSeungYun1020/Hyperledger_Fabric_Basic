...
이 샘플에서는 아래 사용 사례를 이용하여 Org1과 Org2 간에 자산을 전송하기 위해 세 가지 개인 데이터 컬렉션
`assetCollection`, `Org1MSPPrivateCollection`, `Org2MSPPrivateCollection`을 사용하는 방법을 보여준다.

Org1의 멤버는 이제부터 소유자라고 하는 새 자산을 만든다.
소유자의 신원을 포함한 자산의 공개된 세부 정보는 `assetCollection` 개인 데이터 걸렉션에 저장된다.
자산은 소유자가 제공한 평가 가치로 생성된다.
감정가는 각 참여자가 자산 양도에 동의하기 위해 사용하며, 소유자 Organization의 컬렉션에만 보관된다.
우리의 경우 소유자가 동의한 초기 감정가가 `Org1MSPPrivateCollection`에 저장된다.

자산을 구매하려면 구매자가 자산 소유자와 동일한 감정가에 동의해야 한다. 이 단계에서 구매자(Org2의 멤버)는
스마트 컨트랙트 함수 `AgreeToTransfer`을 사용하여 거래 계약을 생성하고 감정 가치에 동의할 수 있다.
이 값은 `Org2MSPPrivateCollection` 컬렉션에 저장된다.
이제 자산 소유자는 스마트 컨트랙트 함수 `TransferAsset`를 사용하여 자산을 구매자에게 양도할 수 있다.
이 `TransferAsset` 함수는 채널 원장의 해시를 사용하여 자산을 이전하기 전에 소유자와 구매자가
동일한 평가 가치에 동의하였는지 확인한다.

전송 시나리오를 진행하기 전에 조직이 패브릭에서 개인 데이터 컬렉션을 사용하는 방법에 대해 알아보자.